...
TEST (ParseExample, DeclareConstant) {
  ...
  std::string funname
    = AST::ptr_cast<AST::Identifier> (fundef->name)->identifier;
...
TEST (ParseExample, DeclareSimpleFunction) {
  ...
  AST::FuncallPtr funcall = AST::ptr_cast<AST::Funcall> (fundef->body);
  std::string     funname2
    = AST::ptr_cast<AST::Identifier> (funcall->name)->identifier;
  ASSERT_EQ (funname2, "add");
  ASSERT_EQ (funcall->args.size ( ), 2);
  auto& lhs
    = AST::ptr_cast<AST::Identifier> (funcall->args.front ( ))->identifier;
  ASSERT_EQ (lhs, "#0");
  auto& rhs
    = AST::ptr_cast<AST::Identifier> (funcall->args.back ( ))->identifier;
  ASSERT_EQ (rhs, "#0");
}
