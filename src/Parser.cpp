...
bool Parser::isAtEnd ( ) {
  if (current.type == TokenType::TOKEN_EOF) { return true; }
  return false;
}
...
AST::BodyPtr Parser::body ( ) {
  ...
  return std::make_shared<AST::Body> (std::move (statements), 0, 0);
}
...
AST::StatementPtr Parser::statement ( ) {
  if (matches (TokenType::IDENTIFIER)) {
    ...
    if (matches (TokenType::ARROW_LEFT)) {
      ...
      return fundef (identifier, previous.line, previous.column);
    } else {
      return funcall (identifier, previous.line, previous.column);
    }
  }
...
AST::FundefPtr Parser::fundef (AST::IdentifierPtr name, u32 line, u32 column) {
  consume (TokenType::ARROW_LEFT, "Expected '<-', found");
  auto functionDefinition = expression ( );
  return std::make_shared<AST::Fundef> (name, functionDefinition, line, column);
}

AST::FuncallPtr
  Parser::funcall (AST::IdentifierPtr name, u32 line, u32 column) {
  ...
  while (!isAtEnd ( ) && !matches (TokenType::RIGHT_PAREN)) {
    ...
    args.push_back (expr);
    if (matches (TokenType::RIGHT_PAREN)) break;
    advance ( );
  ...
  return std::make_shared<AST::Funcall> (name, std::move (args), line, column);
}
...
AST::NumberPtr Parser::number ( ) {
  ...
  return std::make_shared<AST::Number> (value, previous.line, previous.column);
}
